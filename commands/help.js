...
class Help extends Command {
  ...
  async run(message, [type, page=1]) {
    ...
    if (this.client.commands.has(type) || this.client.aliases.has(type)) {
      ...
      helpembed.setTitle(`Command ${cm.name}`)
        .addField('Description', cm.description,true)
        .addField('Description', cm.extended,true)
        .addField('Usage', `\`${cm.usage}\``,true)
        .addField('Aliases', cm.aliases.constructor.name === 'String' ? cm.aliases : cm.aliases.join(', '),true);
    }
